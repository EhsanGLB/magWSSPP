...
)
{
    ...
    forAll(magWSSPP.boundaryField(), patchI)
    {
        magWSSPP.boundaryField()[patchI] = rho.value() * mag(
        (
...
)
{
    ...
    forAll(magWSSPP.boundaryField(), patchI)
    {
        magWSSPP.boundaryField()[patchI] = mag(
        (
           -mesh.Sf().boundaryField()[patchI]
           /mesh.magSf().boundaryField()[patchI]
        ) & Reff.boundaryField()[patchI] );
...
int main(int argc, char *argv[])
{
    ...
    forAll(timeDirs, timeI)
    {
        ...
        if (UHeader.headerOk())
        {
            ...
            if (compressible)
            {
                calcCompressible(mesh, runTime, U, magWSSPP);
            }
            else
            {
                calcIncompressible(mesh, runTime, U, magWSSPP);
            }
        }
        else
        {
        ...
        Info<< "Writing magnitude of wall shear stress to field " << magWSSPP.name() << nl << endl;
